...
static char *teste_tx_deve_iniciar_em_idle(void)
{
    ...
    uint8_t dummy;
    verifica("TX FSM deveria iniciar em IDLE", protocolo_tx_gerar_byte(&tx, &dummy) == false);
    return 0;
}
...
static char *teste_tx_deve_gerar_pacote_completo_na_ordem(void)
{
    ...
    verifica("TX FSM deveria voltar para IDLE apos transmissao", protocolo_tx_gerar_byte(&tx, &byte_gerado) == false);
    return 0;
}
